...
请将 `your-secret-key` 替换为任意强随机字符串。它就是稍后你要在 Open WebUI 中填写的 API key。
...
| **API Key** | 第 2 步中设置的 `API_SERVER_KEY` |
...
请确认 Open WebUI 中填写的 **API Key** 与 `~/.hermes/.env` 中的 `API_SERVER_KEY` 完全一致。
