...
父页面会返回 `{ type: 'payload', requestId: ..., payload: ... }`，其中包含已配置好的负载数据。

### 仅限工具的参数注入

当 **Tool** 返回富 UI 嵌入时，工具调用参数（也就是模型传给工具的参数）会自动注入到 iframe 的 `window.args` 中。这样你的嵌入 HTML 就能访问工具输入：
...
:::note
这只适用于通过工具调用显示渲染的 Tool 嵌入。Action 嵌入没有 `window.args`，因为它们是由用户而不是模型触发的。
:::
