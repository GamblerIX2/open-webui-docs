...
用户也可以在自己的设置中覆盖全局默认值：
- **Default Knowledge Base**：设置一个偏好的 KB，避免每次手动选择
- **Skip Confirmation**：启用后可实现即时一键保存（要求已设置默认 KB）
- **File Name Prefix**：自定义保存来源时的文件名前缀

:::tip 高级用法
在你的 User Valves 中设置 **Default Knowledge Base**，并启用 **Skip Confirmation**，即可实现真正的一键式即时保存网页来源！
:::
