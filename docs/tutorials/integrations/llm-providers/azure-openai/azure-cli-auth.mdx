...
本指南说明如何通过 Azure CLI 和 Entra ID authentication，让 Open WebUI 对 Azure OpenAI 完成认证。
...
- **RBAC Role**：你的用户或用户组必须对 Azure OpenAI 实例拥有 `Cognitive Services OpenAI User` 角色

## 认证步骤

### 1. 通过 Azure CLI 登录

运行以下命令，使用你的 Azure subscription 完成认证：
...
请确保你的用户账号或用户组已经对 Azure OpenAI 资源分配了 `Cognitive Services OpenAI User` 角色。可在 Azure Portal 中确认：

1. 打开你的 Azure OpenAI resource
...
5. 配置 Azure OpenAI endpoint 和 deployment 细节
6. 点击保存
