...
- **Node.js 22+**（推荐 24）
- 来自受支持 LLM 提供商的 **API key**（Anthropic、OpenAI 等）
...
| **API Key** | 你的 OpenClaw Gateway bearer token |
...
2. 进入 **Admin Panel** → **Users**。
3. 为机器人创建一个新账户（例如 `openclaw-bot@yourdomain.com`）。
4. 将该机器人用户加入你希望它监听的频道。

:::note
请确认你的 Open WebUI 实例已经启用了 **Channels**（**Admin Settings** → **General**）。
:::
...
| 设置项 | 填写内容 |
|---|---|
| **Base URL** | 你的 Open WebUI 地址（例如 `http://localhost:3000`） |
| **Email** | 第 1 步中创建的机器人邮箱 |
| **Password** | 机器人账户密码 |
| **Channel IDs** | （可选）要监听的特定频道 ID；留空则监听全部 |
...
- 确认机器人用户已加入目标频道
- 确认实例已在 **Admin Settings** → **General** 中启用 Channels
