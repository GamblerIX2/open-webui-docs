...
Open WebUI 的网络搜索已经从简单的结果注入，演进为完整的 **agentic research system**。当你启用 **Native Function Calling (Agentic Mode)** 后，高质量模型就可以自主探索网页、校验事实并跟进链接。
...
|---------|---------------------------|------------------------------|
| **Search Decision** | 由 Open WebUI 基于提示词分析决定是否搜索 | 由**模型**自己决定是否需要搜索、何时搜索 |
| **Data Processing** | 拉取全部结果、切块并执行 **RAG** | 直接返回 **Snippets**；不切块，也不使用 Vector DB |
| **Link Following** | 将顶部结果的 snippet 注入上下文 | 模型使用 `fetch_url` 直接读取**整页内容** |
| **Model Context** | 仅获取相关片段（Top-K chunks） | 通过 `fetch_url` 获取**整段文本**（最高约 50k 字符） |
| **Reasoning** | 模型在系统注入数据后再处理 | 模型可以搜索、阅读、核查，然后再次搜索 |
...
**用户提问：** "What are the latest security vulnerabilities in the React ecosystem?"
...
- **故障排查**：若遇到问题，请查看 [Web Search Troubleshooting Guide](/troubleshooting/web-search)
