...
1. 点击你的 **profile icon**（左下角）
2. 选择 **Admin Panel**
3. 点击顶部导航中的 **Settings**
4. 选择 **Audio** 标签

你可以在这里配置：
- **Speech-to-Text Engine** —— 在本地 Whisper、OpenAI、Azure、Deepgram、Mistral 之间选择
- **Whisper Model** —— 为本地 STT 选择模型大小（tiny、base、small、medium、large）
- **Text-to-Speech Engine** —— 在 OpenAI-compatible、Mistral、ElevenLabs、Azure、本地 Transformers 或禁用后端 TTS（仅浏览器）之间选择
- **TTS Voice** —— 选择默认音色
- **API Keys and Base URLs** —— 配置外部服务连接
...
用户级选项包括：
- **STT Engine Override** —— 选择 “Web API” 使用浏览器语音识别
- **STT Language** —— 设置偏好的转录语言
- **TTS Engine** —— 选择 “Browser Kokoro” 以使用浏览器本地 TTS
- **TTS Voice** —— 选择可用音色
- **Auto-playback** —— 自动播放 AI 回答
- **Playback Speed** —— 调整播放速度
- **Conversation Mode** —— 启用免手持语音交互
...
如果你有 OpenAI API key，这是最简单的配置方式：

**在 Admin Panel → Settings → Audio 中：**
- **STT Engine:** `OpenAI` | **Model:** `whisper-1`
- **TTS Engine:** `OpenAI` | **Model:** `tts-1` | **Voice:** `alloy`
- 在两个部分都填入你的 OpenAI API key
...
**在 User Settings → Audio 中：**
- **STT Engine:** `Web API`（使用浏览器内置语音识别；**不会调用后端 STT endpoint**）
- **TTS Engine:** `Web API` 或 `Browser Kokoro`（使用浏览器内置 TTS 或客户端侧 Kokoro；**不会调用后端 TTS endpoint**）
...
**检查清单：**
1. 确认 **Admin Panel → Settings → Audio** 中的 TTS 引擎配置正确
...
#### 2. 手动设置 Compute Type
如果你正在使用受影响版本，或在 GPU 上仍然遇到问题，请显式将 compute type 设为 `float16`：
...
#### 调整 Whisper Compute Type

如果你仍希望保留 GPU 加速，可尝试调整 compute type：
...
**可用 compute type（来自 faster-whisper）：**
...
3. **检查浏览器麦克风权限**（见上文）
4. **改用 Web API 引擎：**
   - 进入用户设置（不是 Admin Panel）
   - 在 STT Settings 中把 Speech-to-Text Engine 改为 “Web API”
   - 这样会使用浏览器内置语音识别
...
1. 前往 **Admin Panel → Settings → Audio**
2. 将 TTS 引擎选择为 **ElevenLabs**
3. 输入你的 ElevenLabs API key
4. 选择音色与模型
5. 保存设置
...
environment:
  ...
  - AUDIO_TTS_API_KEY=sk_...  # 你的 ElevenLabs API key
  - AUDIO_TTS_VOICE=EXAVITQu4vr4xnSDxMaL  # ElevenLabs 面板中的 Voice ID
  - AUDIO_TTS_MODEL=eleven_multilingual_v2
```

:::note
你可以在 ElevenLabs 控制台的 voice 设置中找到 Voice ID。常见模型选项包括 `eleven_multilingual_v2` 和 `eleven_monolingual_v1`。
:::
...
| `AUDIO_TTS_API_KEY` | ElevenLabs 或 Azure TTS 的 API key |
| `AUDIO_TTS_OPENAI_API_BASE_URL` | OpenAI-compatible TTS 的 Base URL |
| `AUDIO_TTS_OPENAI_API_KEY` | OpenAI-compatible TTS 的 API key |
| `AUDIO_TTS_MISTRAL_API_KEY` | Mistral TTS 的 API key |
...
| `WHISPER_COMPUTE_TYPE` | Compute type：`int8`、`float16`、`int8_float16`、`float32`（默认：`int8`） |
...
| `AUDIO_STT_OPENAI_API_BASE_URL` | OpenAI-compatible STT 的 Base URL |
| `AUDIO_STT_OPENAI_API_KEY` | OpenAI-compatible STT 的 API key |
| `DEEPGRAM_API_KEY` | Deepgram API key |

完整的音频环境变量列表请参阅 [Environment Variable Configuration](/reference/env-configuration#audio)。
