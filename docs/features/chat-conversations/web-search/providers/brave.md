...
- 在 Admin Panel > Settings > Web Search 中，将 “Concurrent Requests” 设为 `1`；或者直接使用环境变量
...
- **何时会成为问题**：如果多个用户在同一时刻同时触发 web search（例如 3 个用户在同一秒搜索），Open WebUI 会并行处理这些请求。每个用户请求会建立自己的连接池，这意味着 3 个请求会同时被发往 API，从而在免费层下触发限流

**注意：** 如果你运行的是一个有多个并发用户 активно 使用 web search 的环境，强烈建议升级到付费 API 层。免费层并不是为多用户吞吐量设计的。
:::
