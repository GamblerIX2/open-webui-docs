...
**图像提示词生成** 开关位于 **Admin Panel > Settings > Images** 的 “图像生成” 区域中，并且在选择图像生成引擎之前就可以设置。
...
2. **配置图像编辑：**
  - 在 **创建图像** 部分，将 **图像生成引擎** 设为 `ComfyUI`
  - **模型**：选择用于生成图像的基础模型
  - **图像尺寸**：定义输出分辨率（例如 512x512、1024x1024）
  - **步数**：采样步数；数值越高，图像质量通常越高，但生成时间也更长
  - 在 **ComfyUI 基础 URL** 字段中输入运行中的 ComfyUI 地址（例如 `http://host.docker.internal:8188/`）
  - 点击 URL 字段旁的 **刷新图标**（🔄）验证连接；成功后应出现提示
  - 如果你的 ComfyUI 实例需要 API key，请在 **ComfyUI API 密钥** 字段中填写
    | Parameter | Default Key | Required | Description |
    |-----------|-------------|----------|-------------|
  - 首先，你需要从 ComfyUI 导出正确格式的 workflow。在 ComfyUI 界面左上角点击 ComfyUI logo，再点击 **设置**，搜索并启用 **“开发者模式”** 选项，其说明通常为 **“启用开发者模式选项（API 保存等）”**
    | Image* | `image` | Yes | 待编辑的输入图像 |
    | Prompt* | `prompt` | Yes | 编辑提示词文本 |
  - 仍在 ComfyUI 中时，加载你想使用的**图像生成** workflow，然后点击 ComfyUI logo，在 **文件** 子菜单中选择 **“导出（API）”**。系统会提示你为文件命名，请使用一个方便识别的名称并下载
    | Model | `unet_name` | No | Diffusion 模型名 |
    | Width | `width` | No | 输出图像宽度 |
  - 返回 Open WebUI，在 **ComfyUI Workflow** 区域点击 **上传**，然后选择刚刚下载的 JSON workflow 文件
    | Height | `height` | No | 输出图像高度 |

    **注意：** 与图像生成不同，图像编辑不支持 negative prompt、steps、seed 或 batch size 参数。
  | Prompt* | `text` | 是 | 正向提示词文本 |
  | Model | `ckpt_name` | 否 | Checkpoint 模型名 |
  | Width | `width` | 否 | 输出图像宽度 |
  | Height | `height` | 否 | 输出图像高度 |
  | Steps | `steps` | 否 | 采样步数 |
  | Seed | `seed` | 否 | 用于复现的随机种子 |
