...
vLLM 提供**兼容 OpenAI 的 API**（Chat Completions），因此可以很容易接入 Open WebUI。本指南将展示如何连接你的 vLLM 服务器。
...
从 Model Selector 中选择 vLLM 服务器提供的任意模型，然后开始聊天。
...
如果你保存了一个不可达 URL，导致界面卡住，请查看 [Model List Loading Issues](/troubleshooting/connection-error#️-model-list-loading-issues-slow-ui--unreachable-endpoints) 故障排查页面。
