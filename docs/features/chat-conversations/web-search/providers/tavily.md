...
- **已安装 Open WebUI**：一个可运行的 Open WebUI 实例（本地或 Docker）。参阅 [入门指南](https://docs.openwebui.com/getting-started)
- **Tavily 账号**：以及来自 [Tavily](https://app.tavily.com/sign-in) 的 API key
- **管理员权限**：对 Open WebUI 实例具备管理权限
- **网络连接**：用于发起 Tavily API 请求
- **WEBUI_URL 环境变量**：已正确指向你的 Open WebUI 实例。参阅 [环境变量配置](https://docs.openwebui.com/environment)
...
- **环境变量**：可在 `.env` 文件或 Docker 命令中设置 `TAVILY_API_KEY`：
  ...
  参阅 [环境变量配置](https://docs.openwebui.com/environment)
- **RAG 集成**：你可以将 Tavily 结果与本地数据结合，构建 Retrieval Augmented Generation 工作流。参阅 [RAG 文档](https://docs.openwebui.com/features/chat-conversations/rag)
...
- [Open WebUI 功能](https://docs.openwebui.com/features)：RAG 与 web search 说明
- [为 Open WebUI 贡献](https://docs.openwebui.com/contributing)：提交改进或反馈问题
