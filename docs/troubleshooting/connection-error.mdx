...
# 如果你已经启动过 Open WebUI 也不用担心，仍然可以在 admin panel 中修改
...
- 在 **Admin Panel > Settings > WebUI URL** 中修改
- 在首次启动前就正确设置
...
1. 进入 **Admin Settings → Connections**
...
如果你保存了不可达 URL，导致连 Settings 页面都进不去，那么坏配置已经写入数据库，并且会优先于环境变量生效。可以使用以下恢复方式之一：
...
更多信息请参阅 [Environment Configuration](/reference/env-configuration#aiohttp_client_timeout_model_list)。
