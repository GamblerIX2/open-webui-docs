...
function CopyButton({ code }) {
    ...
    return (<button type="button" aria-label={copied ? "Copied" : "Copy code to clipboard"} title="Copy" className={clsx("clean-btn", "shiki-copy-button")} onClick={handleCopy}>
			{copied ? (<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2">
					<polyline points="20 6 9 17 4 12"/>
				</svg>) : (<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2">
...
export default function CodeBlock({ children: rawChildren, ...props }) {
    ...
    if (typeof rawChildren === "string") {
        return (<StringContent key={String(isBrowser)} {...props}>
				{rawChildren}
			</StringContent>);
    }
    // Element children (Shiki output)
    const code = extractText(rawChildren);
    return (<div className="theme-code-block shiki-code-block" key={String(isBrowser)}>
