...
#### 🌐 Q：为什么我的本地 OpenAPI tool server 无法从 WebUI 界面访问？

**A：** 如果你的 tool server 运行在本地（例如 `http://localhost:8000`），基于浏览器的客户端可能会因为 CORS（Cross-Origin Resource Sharing）策略而无法访问。

请确保在你的 OpenAPI server 中显式启用 CORS headers。比如，如果你使用 FastAPI，可以这样配置：
...
此外，如果 Open WebUI 本身通过 HTTPS 提供服务（例如 `https://yourdomain.com`），你的本地 server 还必须满足以下任一条件：

- 通过同域 HTTPS 被访问（例如 `https://localhost:8000`）
- 或者运行在 localhost（`127.0.0.1`）上，以便浏览器对本地开发放宽安全限制
- 否则，浏览器可能会因 mixed-content 规则，阻止 HTTPS 页面向 HTTP API 发起不安全请求
...
**A：** 不需要！虽然我们的参考实现使用 FastAPI 编写，以便更清晰和更容易上手，但你可以使用任何能够产出合法 OpenAPI（Swagger）规范的框架或语言。常见选择包括：
...
同时，建议为所有 endpoints 显式设置自定义 `operationId`。
...
- ✅ **复用现有代码**：如果你已经写过 REST API，那么你几乎已经完成大半工作了——只需要定义一个合规的 OpenAPI spec，并把现有逻辑暴露为 tool server

  而 MCP 则往往要求你把现有工具逻辑重新包进一层自定义协议，造成重复实现并增加维护面
...
#### ❓ Q：通过 OpenAPI tool server 能构建哪些类型的工具？
...
#### 🔌 Q：我可以同时运行多个 tool server 吗？

**A：** 当然可以。每个 tool server 都独立运行，并暴露自己的 OpenAPI schema。你的 agent 配置可以同时指向多个 tool servers，按需混用。

没有硬性数量限制——只需确保每个 server 运行在独立端口或地址上，并且 agent host 可以访问即可。

---

#### 🧪 Q：在把 tool server 挂到 LLM agent 之前，怎么先测试它？

**A：** 你可以使用以下工具来测试 OpenAPI tool servers：
...
#### 🌍 Q：我可以把 OpenAPI tool servers 部署到 AWS、GCP 之类的云平台吗？
